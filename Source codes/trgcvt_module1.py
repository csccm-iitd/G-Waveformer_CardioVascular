...
class CvTencoderdecoder1(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 12, kernels=[5, 3, 2], strides=[2, 2, 2],
    ...
    def forward(self, img1, img2):
        img1 = rearrange(img1, 'b c h w u -> b (h w u) c', h=self.image_size, w=self.image_size, u=self.image_size)
        img2 = rearrange(img2, 'b c h w u -> b (h w u) c', h=self.image_size, w=self.image_size, u=self.image_size)
        ...
        xsr = rearrange(xs3,'b (h w u) c -> b c h w u', h = (self.image_size), w = (self.image_size), u = (self.image_size))
...
class CvTencoderdecoder2(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 12, kernels=[4,3, 2], strides=[2, 2, 2],
                 heads=[1, 3, 6] , depth = [1, 2, 10], pool='cls', dropout=0., emb_dropout=0., scale_dim=4):
        super().__init__()
        ...
        ##### Stage 1 #######
        self.stage1enc_conv_embed = nn.Sequential(
            nn.Conv3d(in_channels1, dim, kernels[0], strides[0], 1),
            Rearrange('b c h w u -> b (h w u) c', h = (self.image_size+1)//2, w = (self.image_size+1)//2,  u = (self.image_size+1)//2),
            nn.LayerNorm(dim)
        )

        
        self.stage1dec_conv_embed = nn.Sequential(
            nn.Conv3d(in_channels2, dim, kernels[0], strides[0], 1),
            Rearrange('b c h w u -> b (h w u) c', h = (self.image_size+1)//2, w = (self.image_size+1)//2, u = (self.image_size+1)//2),
            nn.LayerNorm(dim)
        )
        
        self.stage1enc_transformer = nn.Sequential(
            Transformer1(dim=dim, img_size=(self.image_size+1)//2,depth=depth[0], heads=heads[0], dim_head=self.dim,
                                              mlp_dim=dim * scale_dim, dropout=dropout),
            Rearrange('b (h w u) c -> b c h w u', h = (self.image_size+1)//2, w = (self.image_size+1)//2,  u = (self.image_size+1)//2)
        )
        ...
        self.stage2enc_conv_embed = nn.Sequential(
            ...
            Rearrange('b c h w u -> b (h w u) c', h = (self.image_size+3)//4, w = (self.image_size+3)//4, u = (self.image_size+3)//4),
            nn.LayerNorm(dim)
        )
        ...
        self.stage2enc_transformer = nn.Sequential(
            Transformer1(dim=dim, img_size=(image_size+3)//4, depth=depth[1], heads=heads[1], dim_head=self.dim,
                                              mlp_dim=dim * scale_dim, dropout=dropout)
        )
        ...
        # dim = scale*dim
        # dim = 36
        self.stage3_transformer = Transformer2(dim= dim, img_size=(image_size+3)//4, depth=depth[1], heads=4, dim_head=self.dim,
                                              mlp_dim=dim * scale_dim, dropout=dropout)
        ...
        self.stage4_conv_embed = nn.ConvTranspose3d(dim, 32, kernels[1], strides[1], padding=1)
        
        self.stage5_conv_embed = nn.ConvTranspose3d(32, out_dim, kernels[0], strides[0], padding=1)
    ...
    def forward(self, img1, img2):
        ...
        xsr = rearrange(xs3,'b (h w u) c -> b c h w u', h = (self.image_size+3)//4, w = (self.image_size+3)//4, u = (self.image_size+3)//4)
