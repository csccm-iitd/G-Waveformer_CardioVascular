...
class ConvAttention1(nn.Module):
    def __init__(self, dim, img_size, heads = 8, dim_head = 16, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout = 0.,
                 last_stage=False):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b (l w u) n -> b n l w u', l=self.img_size, w=self.img_size, u =self.img_size)

        q = self.to_q(x)
        q = rearrange(q, 'b (h d) l w u -> b h (l w u) d', h=h)

        v = self.to_v(x)
        v = rearrange(v, 'b (h d) l w u -> b h (l w u) d', h=h)

        k = self.to_k(x)
        k = rearrange(k, 'b (h d) l w u -> b h (l w u) d', h=h)
...
class ConvAttention2(nn.Module):
    ...
    def forward(self, x,y):
        ...
        x = rearrange(x, 'b (l w u) n -> b n l w u', l= self.img_size, w= self.img_size, u = self.img_size)
        y = rearrange(y, 'b (l w u) n -> b n l w u', l= self. img_size, w= self.img_size, u = self.img_size)

        q = self.to_q(y)
        q = rearrange(q, 'b (h d) l w u -> b h (l w u) d', h=h1)
