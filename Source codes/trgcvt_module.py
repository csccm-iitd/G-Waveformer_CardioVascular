...
class CvTencoderdecoder1(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 9, kernels=[5, 3, 2], strides=[2, 2, 2],
                 heads=[4, 4, 6] , depth = [1, 2, 10], pool='cls', dropout=0., emb_dropout=0., scale_dim=4):
        super().__init__()
...
class CvTencoderdecoder2(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 9, kernels=[6,3, 2], strides=[2, 2, 2],
                 heads=[4, 4, 6] , depth = [1, 2, 10], pool='cls', dropout=0., emb_dropout=0., scale_dim=4):
        super().__init__()
        ...
        self.stage1enc_conv_embed = nn.Sequential(
            ...
            Rearrange('b c h w u -> b (h w u) c', h = (self.image_size-2)//2, w = (self.image_size-2)//2,  u = (self.image_size-2)//2),
            nn.LayerNorm(dim)
        )
        ...
        self.stage1enc_transformer = nn.Sequential(
            Transformer1(dim=dim, img_size=(self.image_size-2)//2,depth=depth[0], heads=heads[0], dim_head=self.dim,
                                              mlp_dim=dim * scale_dim, dropout=dropout)
        )
        ...
        # dim = scale*dim
        # dim = 36
        self.stage3_transformer = Transformer2(dim= dim, img_size=(self.image_size-2)//2, depth=depth[1], heads=4, dim_head=self.dim,
                                              mlp_dim=dim * scale_dim, dropout=dropout)
        ...
        self.stage5_conv_embed = nn.ConvTranspose3d(dim, out_dim, kernels[0], strides[0], padding=1)
    ...
    def forward(self, img1, img2):
        ...
        xsr = rearrange(xs3,'b (h w u) c -> b c h w u', h = (self.image_size-2)//2, w = (self.image_size-2)//2, u = (self.image_size-2)//2)
